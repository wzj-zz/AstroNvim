...
- `<C-g>`：当前文件符号大纲
- `<Leader>lt`：查看语法树
- `s`：Flash 拼音跳转；Normal / Visual 模式下实时筛选当前屏幕可见范围内的可搜索串，支持中文拼音、首字母和英文/数字混合搜索；直接使用标签跳转
- `r`：Operator-pending 模式下使用 Flash 拼音远程选择操作目标
...
- `<C-c>`：取消当前任务
- `<Tab>`：切换 pane
- `<M-n>` / `<M-p>`：在 USER 消息之间向下或向上跳转
- `<M-N>` / `<M-P>`：跳到下一个或上一个 snapshot 标记
- `a` / `A`：接受权限请求
- `d`：拒绝权限请求
- `<M-t>`：切换 tool output
- `<M-r>`：切换 reasoning output
...
- `<Leader>,x`：在右侧半屏终端运行当前文件或选中代码；输出窗口默认 normal 模式，按 `q` 可关闭
